...
const emptyForm = { nombre: '', categoria: '', unidad: 'unidad', stock_actual: '', stock_minimo: '', costo_unitario: '', notas: '' }
...
export default function Materiales() {
  ...
  async function handleSave() {
    setSaving(true)
    const payload = { ...form, stock_actual: Number(form.stock_actual) || 0, stock_minimo: Number(form.stock_minimo) || 0, costo_unitario: Number(form.costo_unitario) || 0 }
  ...
  return (
    <div className="space-y-5">
            ...
            return (
              <div key={m.id} className={`card relative overflow-hidden ${lowStock ? 'border-red-200 bg-red-50/30' : ''}`}>
                ...
                <div className="flex gap-1 mt-3 pt-3 border-t border-gray-100">
                  <button onClick={() => openEdit(m)} className="flex-1 btn-secondary text-xs py-1.5 justify-center">
                    <Pencil size={12} /> Editar
                  </button>
                  <button onClick={() => handleDelete(m.id)} className="btn-danger text-xs py-1.5 px-2">
                    <Trash2 size={12} />
                  </button>
                </div>
              </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Editar material' : 'Nuevo material'}>
